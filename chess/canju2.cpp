...
#include <windows.h>
canju2::canju2(QWidget *parent) : Board(parent)
{
    //this->resize(561,557);
    QPalette palette;
...
void canju2::drawStone(QPainter & p, int id)
{
    if(_s[id]._dead||id>=4)
    {
        return;
    ...
    if(id==2)p.drawImage(rect,pic[16]);
    if(id==3)p.drawImage(rect,pic[16]);
}
