...
#include <windows.h>
canju3::canju3(QWidget *parent) : Board(parent)
{
    //this->resize(561,557);
    QPalette palette;
...
void canju3::drawStone(QPainter & p, int id)
{
    if(_s[id]._dead||id>=4)
    {
        return;
    ...
    if(id==1)p.drawImage(rect,pic[1]);
    if(id==2)p.drawImage(rect,pic[20]);
    if(id==3)p.drawImage(rect,pic[19]);
}
